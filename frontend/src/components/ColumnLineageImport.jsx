...
const ColumnLineageImport = ({ visible, onCancel, onSuccess }) => {
  ...
  const handleJsonUpload = async (file) => {
    try {
      ...
      console.error('字段血缘关系JSON文件上传失败:', error);
      if (error.response?.data?.detail) {
        showErrorDetail(error.response.data.detail);
        message.error('导入失败，请查看错误详情');
      } else {
        message.error('字段血缘关系JSON文件上传失败，请重试');
      }
    } finally {
      setUploading(false);
    }
        ...
        <>
          <div style={{ marginBottom: '16px', padding: '12px', backgroundColor: '#e6f7ff', borderRadius: '4px' }}>
            ...
            <span style={{ marginLeft: '8px', color: '#1890ff' }}>
              注意：导入前系统将自动验证源字段、目标字段及其所属表是否已存在于系统中。请确保相关表结构和字段已先导入。
            </span>
          </div>
...
export default ColumnLineageImport;